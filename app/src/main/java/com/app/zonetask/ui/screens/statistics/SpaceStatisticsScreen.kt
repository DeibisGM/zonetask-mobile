...
fun SpaceStatisticsScreen(
    ...
    viewModel: SpaceStatisticsViewModel = viewModel(
        factory = SpaceStatisticsViewModelFactory(spaceId)
    )
) {
...
@Composable
private fun SpaceStatisticsContent(
    stats: SpaceStatisticsResponse,
    modifier: Modifier = Modifier
) {
    LazyColumn(
        modifier            = modifier,
        ...
        item {
            SpaceCompletionRateCard(rate = stats.completionRate)
        }
...
@Composable
private fun SpaceCompletionRateCard(rate: Double) {
    val rateColor = when {
